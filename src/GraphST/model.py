...
# ========================================================================================
# 🌟 [Innovation V30] Node-Adaptive Dual-Graph Fusion Module
# ========================================================================================
class BoundaryAwareGraphRefiner(nn.Module):
    """
    Boundary-aware graph refinement module.

    功能：
    1. 根据 latent embedding 计算节点间语义相似度
    2. 仅在物理邻域范围内保留边（physical mask）
    3. 用 ReLU + power(gamma) 做非线性锐化
    4. 输出归一化后的动态图

    参数：
    - adj_mask: 二值物理邻接 mask, shape [N, N]
    - dim_hidden: latent embedding 维度
    - gamma: 锐化指数，默认 3.0
    - use_proj: 是否使用可学习投影头
    """

    def __init__(self, adj_mask, dim_hidden, gamma=3.0, use_proj=True):
        super(BoundaryAwareGraphRefiner, self).__init__()
        self.register_buffer("adj_mask", adj_mask)
        self.gamma = gamma
        self.use_proj = use_proj

        if self.use_proj:
            self.proj = nn.Linear(dim_hidden, dim_hidden, bias=False)

    def forward(self, z):
        """
        z: latent embeddings, shape [N, d]
        return: refined normalized adjacency, shape [N, N]
        """
        if self.use_proj:
            z = self.proj(z)

        z = F.normalize(z, p=2, dim=1)
        sim = torch.mm(z, z.t())  # cosine similarity because z normalized

        # 只保留物理邻域内的相似度
        sim_local = sim * self.adj_mask

        # ReLU + gamma sharpening
        adj_dyn = torch.pow(F.relu(sim_local), self.gamma)

        # 防止某一行全 0
        row_sum = torch.sum(adj_dyn, dim=1, keepdim=True) + 1e-12
        adj_dyn = adj_dyn / row_sum

        return adj_dyn


# ========================================================================================
# End of Innovation Module
# ========================================================================================

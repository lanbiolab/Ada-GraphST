...
def plot_one_panel(ax, adata, cluster_key, batch_key, batch_value, color_map, title, embed_key):
    ...
    try:
        ...
        if len(unique_labels) > 1 and feat_matrix is not None:
            ...
            # 在图的左下角添加文本标注，字体放大并加粗，添加白底防遮挡
            ax.text(
                0.03, 0.03, f"Silhouette: {sil_score:.3f}",
                transform=ax.transAxes,
                fontsize=16,  # 字体大小，如觉得不够大可以改到 18 或 20
...
def plot_dataset_spatial_comparison(dataset_name, graphst_path, ada_path):
    ...
    print(f"GraphST: n_obs={adata_g.n_obs}, cluster_key={cluster_g}, batch_key={batch_g}, embed_key={embed_g}")
    print(f"Ada-GraphST: n_obs={adata_a.n_obs}, cluster_key={cluster_a}, batch_key={batch_a}, embed_key={embed_a}")
    ...
    # 每个 batch 两列：GraphST / Ada-GraphST
